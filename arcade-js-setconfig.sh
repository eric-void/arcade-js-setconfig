...
if [ ! -f /proc/bus/input/devices ]; then
    >&2 echo "ERROR: /proc/bus/input/devices not found"
    exit 1
fi
if [ ! -f $CFGFILE ]; then
    >&2 echo "ERROR: $CFGFILE not found"
    exit 1
fi
        ...
        if [ "$Name" -a "$Sysfs" -a "$Handlers" ]; then
            ...
            if [ "$SysPath" -a "$JsEvent" ]; then
                ...
                if [[ "$GuidRaw" =~ GUID:[[:space:]]*([a-z0-9]*) ]]; then
                    ...
                    >&2 echo " ... ERROR: GUID not found with sdl2-jstest"
                fi
            else
                >&2 echo " ... ERROR: sysfs or handlers not recognized"
            fi
        else
            echo " ... not found"
        fi
